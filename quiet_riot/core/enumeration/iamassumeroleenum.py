...
def iam_assume_role_princ_checker(rand_account_id):
    policy = {
        "Version": "2012-10-17",
        "Statement": [
            {"Sid": "", "Effect": "Allow", "Principal": {"Federated": rand_account_id}, "Action": "sts:AssumeRole"}
        ],
    }
    try:
        policy_blob = json.dumps(policy)
        response = client.update_assume_role_policy(RoleName="aqua-test-role", PolicyDocument=policy_blob)
